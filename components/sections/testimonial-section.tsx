...
import { Star, ChevronLeft, ChevronRight, Maximize2, Minimize2 } from "lucide-react";
...
export function TestimonialSection() {
  ...
  const checkScrollButtons = useCallback(() => {
    if (scrollRef.current) {
      ...
      const firstCard = scrollRef.current.querySelector("[data-testimonial-card]") as HTMLElement;
      if (firstCard) {
  ...
  return (
    <motion.section
      ...
      <motion.div className="container mx-auto px-4 max-w-7xl" variants={staggerContainer()}>
        {/* Header */}
        <motion.div className="mb-12 md:mb-16 lg:mb-20 text-center" variants={itemFade}>
          <div className="inline-block mb-4 px-4 py-2 bg-primary/10 rounded-full">
            <span className="text-primary font-semibold text-sm md:text-base">Testimoni Jemaah</span>
          </div>

          <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-6">
            Kata <span className="bg-linear-to-r from-primary to-primary/80 bg-clip-text text-transparent">Jemaah Kami</span>
          </h2>

          <p className="text-base md:text-lg lg:text-xl text-gray-600 leading-relaxed max-w-3xl mx-auto">Dengarkan pengalaman sebenar jemaah yang telah menunaikan ibadah haji dan umrah bersama Rayhar Travels</p>
        </motion.div>
        ...
        <motion.div className="relative" variants={itemFade}>
          ...
          >
            {testimonials.map((testimonial) => (
              <motion.div key={testimonial.id} data-testimonial-card className="shrink-0 w-[280px] md:w-[320px] lg:w-[360px] snap-center" variants={scaleIn}>
                <Card
                  onClick={() => {
                ...
                >
                  ...
                  <CardContent className="p-4 md:p-5 flex flex-col flex-1">
                    ...
                    <div className="flex items-center gap-0.5 mb-3">
                      {[...Array(5)].map((_, i) => (
                        <Star key={i} className={`w-3.5 h-3.5 ${i < testimonial.rating ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"}`} />
                      ))}
                    </div>

                    {/* Testimonial Text - dengan truncate */}
                    <p className="text-gray-900 text-sm leading-relaxed mb-4 flex-1 line-clamp-4">{testimonial.text}</p>
                    ...
                    <div className="flex items-center gap-3 pt-3 border-t border-gray-100">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {testimonial.location && <p className="text-gray-500 text-xs truncate">{testimonial.location}</p>}
                      </div>
                    </div>
                  </CardContent>
                </Card>
          ...
          <motion.div className="flex items-center justify-center gap-4 md:gap-6 mt-6" variants={itemFade}>
            {/* Left Arrow */}
            <button
              onClick={() => scroll("left")}
              disabled={!canScrollLeft}
              className={`p-2 md:p-3 rounded-full transition-all duration-300 ${canScrollLeft ? "bg-primary text-white hover:bg-primary/90 hover:scale-110 shadow-lg" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
            ...
            <div className="flex items-center justify-center gap-2.5">
              {Array.from({ length: testimonials.length }).map((_, index) => (
                <button key={`dot-${index}`} onClick={() => scrollToIndex(index)} className="transition-all duration-300 hover:scale-110" aria-label={`Go to slide ${index + 1}`}>
                  <svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg" className={`transition-all duration-300 ${activeIndex === index ? "scale-125" : "scale-100"}`}>
                    <path d="M6 0L7.5 4.5L12 6L7.5 7.5L6 12L4.5 7.5L0 6L4.5 4.5L6 0Z" fill={activeIndex === index ? "hsl(var(--primary))" : "#E5E7EB"} className="transition-all duration-300" />
                  </svg>
                </button>
              ))}
            </div>
            ...
            <button
              ...
              className={`p-2 md:p-3 rounded-full transition-all duration-300 ${canScrollRight ? "bg-primary text-white hover:bg-primary/90 hover:scale-110 shadow-lg" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
      ...
      {isModalOpen && (
        ...
        >
          {/* Overlay */}
          <motion.div className="absolute inset-0 bg-black/50 backdrop-blur-sm" initial={{ opacity: 0 }} animate={{ opacity: 1 }} />
          ...
          <motion.div
            ...
            <div className="p-6">
              {selectedTestimonial && (
                <>
                  {/* Header */}
                  <h2 className="text-2xl font-bold text-gray-900 mb-4">Testimoni Jemaah</h2>
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="flex items-center gap-4 pb-4 border-b border-gray-200">
                      <div className="w-16 h-16 rounded-full bg-linear-to-br from-primary/20 to-primary/40 flex items-center justify-center shrink-0">
                        <span className="text-primary font-bold text-xl">{selectedTestimonial.name.charAt(0)}</span>
                      </div>
                      <div className="flex-1">
                        <h3 className="font-semibold text-lg text-gray-900 mb-1">{selectedTestimonial.name}</h3>
                        {selectedTestimonial.location && <p className="text-gray-600 text-sm">{selectedTestimonial.location}</p>}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <Star key={i} className={`w-5 h-5 ${i < selectedTestimonial.rating ? "fill-yellow-400 text-yellow-400" : "fill-gray-200 text-gray-200"}`} />
                        ))}
                      </div>
                      <span className="text-sm text-gray-600">{selectedTestimonial.rating}.0 / 5.0</span>
                    </div>

                    {/* Full Testimonial Text */}
                    <div className="bg-gray-50 rounded-lg p-6 border border-gray-200">
                      <div className="flex items-start gap-3">
                        <div className="text-primary text-4xl font-serif leading-none mt-1">&ldquo;</div>
                        <p className="text-gray-700 text-base leading-relaxed flex-1">{selectedTestimonial.text}</p>
                        <div className="text-primary text-4xl font-serif leading-none mt-auto">&rdquo;</div>
                      </div>
                    </div>
